...
- adam_path:
  ...
  supplied as CSV (\`.csv\`), SAS transport (\`.xpt\`) or CDISC
  Dataset-JSON (\`.json\`) files; the reader emitted for each dataset is
  chosen from the file extension found on disk (see
  \[.generate_one_adam_read()\]).
