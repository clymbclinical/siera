...
* Extended the per-category risk-difference method (`risk_difference_per_group`) to pre-defined (non-data-driven) inner groupings: because it loops the observed values of the second grouping variable, it applies equally when the grouping declares fixed groups, as in the eTFL "Overview of Adverse Events" table (one risk difference per action taken and per severity). Validated against an independent `stats::prop.test()` recomputation; the published reference values for those analyses were found to be computed from only the first adverse-event record per subject rather than from all qualifying events, so they are not used as the oracle (#171).
