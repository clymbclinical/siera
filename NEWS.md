...
* Extended the per-category risk-difference method to the `fda-ae-t06` eTFL regression table (per AE action and per severity), so those analyses now emit one risk difference and 95% confidence interval per category instead of a single overall value, asserted against an independent `prop.test` ground truth (#171).
