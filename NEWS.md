...
* Added a `blank_rows` argument to `readARS()` (default `FALSE`, preserving prior behaviour). When `TRUE`, categorical analyses grouped by a pre-defined (non-data-driven) set of categories - e.g. an age category of `< 18`, `18 - < 60`, `>= 60` - report a row for every defined category even when no subjects fall into it (`n` = 0), instead of silently dropping the category from the ARD. This is implemented by coercing the grouping variable to a factor with the full set of ARS-defined category levels before calling `cards::ard_tabulate()`, which then completes unobserved levels; data-driven groupings are left untouched since their categories are only known from the data, and a grouping variable that is also the discriminating variable of the analysis's own data subset is likewise left untouched, since its other values are deliberately excluded by that subset rather than merely absent from the data. Also fixed a related latent bug in `group[n]_groupId`/`group[n]_groupValue` stamping: `as.character()` on the list column `cards::ard_tabulate()` returns for `group[n]_level` silently extracted the underlying factor code instead of its label whenever the grouping variable was a factor, which would have produced `NA` group ids for any factor-typed grouping variable (#100).
