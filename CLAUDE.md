...
**Authoring PR/issue body files — do NOT use `/tmp` heredocs.** In the
Bash tool a `cat > /tmp/pr.md <<'EOF' … EOF` heredoc followed by reading
that file in the *same* compound command intermittently fails with
`FileNotFoundError: /tmp/pr.md` (the Git Bash `/tmp` mount is unreliable
across the tool boundary), which then 422s the GitHub API with an
empty/`nil` body. **Robust pattern:** write the body with the **Write
tool** to the session scratchpad (an absolute Windows path), then pass
that path to `python`/`curl`/PowerShell. Build the JSON payload with
`python -c "import json; print(json.dumps({...,'body':open(r'<abs path>',encoding='utf-8').read()}))"`
and POST with `curl`. To **create** a PR send
`title`/`head`/`base`/`body`; if the branch already has an open PR the
create call 422s with “A pull request already exists” — fetch it via
`GET /pulls?head=clymbclinical:<branch>&state=open` and **PATCH**
`/pulls/<num>` to set the body instead.
...
More than one Claude session may be active on this repo at once
(typically one per issue). Sessions that share a single working tree
also share **one index and one HEAD**, so any `git checkout`,
`git stash`, `git reset`, or `git commit` in one session silently
corrupts the other (e.g. a `git stash -u` to start a branch will drag a
parallel session’s uncommitted WIP onto your branch, and your commit can
land on their branch).

- **Before stashing or switching branches, check
  `git branch --show-current` and whether a parallel session is
  mid-edit.** If the working tree has untracked/modified files you did
  not create, a parallel session is likely active — stop and use a
  worktree instead.
- **For genuinely parallel work, give each line of work its own checkout
  with `git worktree`** (branches, commits, and pushes are shared via
  the common `.git`; only the working files are isolated):
  - New branch:
    `git worktree add -b <issue>-<slug> ../siera-<issue> origin/main`
  - Existing branch:
    `git worktree add ../siera-<issue> <existing-branch>`
  - Run that session with its working directory pointed at the new
    folder; edit/commit/push there.
  - Clean up after merge: `git worktree remove ../siera-<issue>`.
  - A branch can be checked out in only one worktree at a time (git
    enforces this). Stagger `devtools::check()`/`install()` across
    worktrees — they share the same installed-package library.
- **Never `git stash` in a shared tree, and never `git add -A`** — stage
  only your own explicit paths.
- **Never `git reset --hard` to undo a misplaced commit in a shared
  tree** — it destroys a parallel session’s modified tracked files. To
  extract a commit onto its own branch without touching the working
  tree: `git branch -f <target-branch> <commit>`;
  `git reset --soft origin/main`;
  `git restore --source=origin/main --staged --worktree -- <only your files>`.
...
**Where the cards API in generated output actually comes from:** the
`cards`/`cardx` function names and argument patterns in generated (and
`inst/script/`) ARDs are copied verbatim from the ARS metadata’s
`AnalysisMethodCodeTemplate.templateCode` — siera only substitutes the
placeholder parameters, it does not rewrite the cards calls.
...
- **eTFL Portal integration testing** — `tests/testthat/testdata/etfl/`
  ...
  - **\#173 — method-template library formalised as plain text
    ...
    **source of truth** is `inst/method-library/` — one folder per
    ...
    no generated xlsx.
  - **\#175 — external method-template references (resolve
    `codeTemplate.documentRef`).** Delivered the “reference a library
    method by id” direction \#173 named. When an ARS method’s
    `codeTemplate` has no inline `code` but carries a `documentRef`,
    siera follows the ARS chain `documentRef.referenceDocumentId` →
    `referenceDocuments[].location` → file, and resolves the template
    (and, when the method has no inline `parameters`, its parameters
    too). Two formats: **Option B** a siera *method manifest* `.json`
    (superset of `method.json` with
    `templateCode`+`parameters`+`operations`; method chosen by
    `pageRefs` `refType:"NamedDestination"` `pageNames:["<id>"]`, or the
    sole method if single-method), and **Option A** a bare `.R`/`.txt`
    code file (params stay inline). Resolution lives in the **metadata
    layer** (`R/method_reference.R`
    [`.resolve_method_documentref()`](https://clymbclinical.github.io/siera/reference/dot-resolve_method_documentref.md) +
    extraction helpers; wired into `R/metadata.R` JSON path ~L400 and
    XLSX path after the AnalysisMethodCodeTemplate read) so
    `R/AnalysisMethods.R`/`readARS.R` are untouched — a `documentRef`
    ARS generates a byte-identical script to its inline twin (JSON path
    exact; XLSX path identical modulo the pre-existing quirk that the
    **inline xlsx reader does not strip `\r`/`_x000D_`** from
    `templateCode`, leaving cosmetic blank lines — the manifest path is
    clean, so the parity test compares blank-line-stripped). v1 is
    **local paths only** (relative to the ARS file dir, or absolute);
    remote URIs are rejected via
    [`.is_remote_uri()`](https://clymbclinical.github.io/siera/reference/dot-is_remote_uri.md).
    Manifest valueSources are validated against
    [`.supported_value_sources()`](https://clymbclinical.github.io/siera/reference/dot-supported_value_sources.md).
    The whole library now also ships as a referenceable catalog
    `inst/method-library/method-library.json` — generated by
    [`.render_method_library_json()`](https://clymbclinical.github.io/siera/reference/dot-render_method_library_json.md)
    (regenerate:
    `writeLines(siera:::.render_method_library_json(), "inst/method-library/method-library.json")`),
    held to a freshness contract test like `METHODS.md`. XLSX parity
    uses the canonical ARS sheets `ReferenceDocuments`
    (id,name,location) + `AnalysisMethodDocumentRefs` (method_id,
    refDocumentId, pageRef_pages) — both already present (empty) in
    TFL-Designer xlsx exports. Bundled runnable examples:
    `inst/extdata/exampleARS_5_documentref.{json,xlsx}` referencing
    `inst/extdata/exampleARS_methods.json` (manifest keyed by
    Mth_01/02/03). Tests in `tests/testthat/test-method-reference.R`
    (resolver units, error paths, both-format parity) + catalog
    freshness in `test-method-library.R`. Two latent robustness fixes
    landed too: (1) an all-blank xlsx `AnalysisMethodCodeParameters`
    sheet reads back as logical columns, so the XLSX path re-types it
    before `bind_rows`; (2) the downstream template filter in
    `AnalysisMethods.R` requires
    `context %in% c("R","R (siera)","siera")`, so the resolver returns a
    `context` (manifest method’s, else manifest-level, else
...
- **Keep documentation and functionality in sync.** Any user-visible
  ...
  know about — must be reflected in the documentation (vignettes,
  README, and `NEWS.md`) as part of the same change, not deferred. Treat
  user-facing docs as part of the definition of done. When in doubt
...
- `_pkgdown.yml` groups the Articles navbar (Get started / Concepts /
  ...
  ungrouped.
- The pipeline diagram is a hand-authored static **SVG** (CRAN-safe, no
  new dependency), committed twice:
  `vignettes/figures/siera-pipeline.svg` (used by `concepts.Rmd` via
  [`knitr::include_graphics()`](https://rdrr.io/pkg/knitr/man/include_graphics.html))
  and `man/figures/siera-pipeline.svg` (used by the README / pkgdown
  home). Update both copies together.
- **Verifying vignettes without Pandoc:** full
  [`rmarkdown::render()`](https://pkgs.rstudio.com/rmarkdown/reference/render.html)
  / `build_vignettes()` needs Pandoc (unavailable locally), but
  `knitr::knit(vig, output = tempfile())` executes all chunks without
  Pandoc — use it to confirm chunks run. `Getting_started.Rmd` has a
  **live** chunk that [`source()`](https://rdrr.io/r/base/source.html)s
  an `inst/script/` example and prints `head(ARD)`, so it requires
  `cards`/`cardx` installed and the example scripts to be current.
- Tone is conversational, second-person, tutorial-style with
  inline-commented chunks; match it when editing docs. Audience is
  clinical-trial practitioners, not R developers.
- `README.Rmd`’s install chunks lack `eval=FALSE`, so re-knitting would
  actually run
  [`install.packages()`](https://rdrr.io/r/utils/install.packages.html)/`install_github()`.
  For small static edits, change `README.Rmd` and `README.md` by hand in
  sync rather than re-knitting.
