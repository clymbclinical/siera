...
- **`n_group_cols` determination** — the number of `group[n]` columns a method produces differs from `num_grp` when the last grouping goes to `variables=` instead of `by=`. Always inspect the *resolved* `code_method` string for the presence of `by_vars`/`strata_vars` or `by_listc` substrings. Do NOT check the parameter table — it can contain spurious unused parameters that give false positives.
