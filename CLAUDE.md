...
- **eTFL Portal integration testing** — `tests/testthat/testdata/etfl/` holds the CDISC eTFL Portal fixtures unzipped into three subfolders (no `.zip` files are committed): `metadata/<table>-siera.json` (ARS metadata enriched with `codeTemplate` + `parameters` so siera emits runnable scripts), `adam/<table>/*.xpt` (the ADaM data for that table), and `reference/<table>-ard.json` (the published reference ARD). `test-etfl-regression.R` runs the full pipeline for each of the 12 tables and compares the computed statistics against the reference ARD; the machinery lives in `helper-etfl.R` (`.run_etfl_pipeline()` reads XPT via `haven::read_xpt()` → CSV → `readARS()` → sources the script; `.load_etfl_reference()`; the `.cmp_bigN/.cmp_n_pct/.cmp_n_pct_2level/.cmp_continuous/.cmp_rd` comparators; and `.expect_all_match()`). Reference ARDs are Dataset-JSON; load with `jsonlite::fromJSON(..., simplifyDataFrame=FALSE, simplifyVector=FALSE)` to avoid row-parsing failures. `fda-ds-t04` derives `DISCONFL` from `DCTREAS` via the `adsl_transform` hook. The `.cmp_pergroup_rd()`/`.etfl_pergroup_rd_truth()` helpers assert per-category RD (#157/#171, see below). The one remaining known-limitation analysis exercised but not asserted: per-(SOC × PT) RD (fda-ae-t36 An_57, a 3-grouping case `Mth_03_1a` doesn't cover, #172). The raw XPT/JSON is ~28 MB in the working tree but git-packs and tarball-gzips to ~2 MB, so it ships fine for CRAN.
  ...
  - **#171 — per-category RD extended to fda-ae-t06 (resolved: repoint to `Mth_03_1a`; another reference defect found).** An_48_2/An_48_3 (RD per action taken, `AEACN`) and An_50_2/An_50_3 (RD per severity, `AESEV`) were repointed from the single-RD `Mth_03_1` to `Mth_03_1a` (block copied verbatim from the t13 metadata into `fda-ae-t06-siera.json`; no R change). Unlike t13/t36, the inner groupings here are **pre-defined** (`dataDriven: false`, groups with `AEACN EQ`/`AESEV EQ` conditions) — `Mth_03_1a` still applies because it loops *observed* Group2 values; it stamps only `group2_level` (no `group[n]` metadata columns — the template has no `by_listc`/`by_vars`/`strata_vars` token, so `n_group_cols = 0`). Defined groups with no matching data (`DOSE DELAY`, `OTHER`) emit no row, and the data value `NOT APPLICALE` (ADAE typo) survives `Dss_68/69`'s `NOTIN ('NOT APPLICABLE', ...)` filter. **The t06 reference ARD is a defective oracle for these four analyses**: its per-category counts are reproduced exactly by collapsing ADAE to *one row per subject (the first row)* before applying subset/group conditions (proof: brute-forced the (x1,x2) behind each reference estimate/CI triple — unique `prop.test(correct=FALSE)` solutions, e.g. An_50_2 arm-1 partition 45 MILD / 30 MODERATE / 2 SEVERE = the first-row severity split, summing to the 77 any-AE subjects; likewise INT 26/15/15, RED 18/19/11 across arms). The ARS conditions are event-level, so a subject counts in every category with a qualifying event — siera matched the independent full-event `.cmp_pergroup_rd()` ground truth on all 22 category × arm-pair values (4+4 actions, 3+3 severities). The library entry `05_risk_difference_per_group` was generalised (name/description note pre-defined groupings; `verified_against` includes t06) and `METHODS.md`/`method-library.json` regenerated.
