...
- **eTFL Portal integration testing** — `tests/testthat/testdata/etfl/` holds the original eTFL Portal ZIPs (never modify). `tests/testthat/testdata/etfl-siera/` holds `*-siera.json` copies with `codeTemplate` + `parameters` entries added to each method so siera can generate runnable scripts. Each eTFL also has a `compare_*.R` script that extracts XPT files from the ZIP, converts them to CSV via `haven::read_xpt()`, runs `readARS()`, sources the generated script, loads the Dataset-JSON reference ARD, and compares key statistics. The reference ARDs use Dataset-JSON format; load with `jsonlite::fromJSON(..., simplifyDataFrame=FALSE, simplifyVector=FALSE)` to avoid row-parsing failures with the default simplification settings.
