...
- **`opidNhere` tokens are substituted by pattern, not only via declared parameters (issue #183)** — the method-library manifests deliberately declare no `operation_N` parameters (the family is derived from each ARS method's own `operations`, in `order`), but the declared-parameter loop in `.generate_analysis_method_section()` was the only substitution path, so documentRef-resolved library methods leaked literal `opid1here`… into generated scripts/ARDs. Fixed by a pattern pass after the declared loop: for each `operations_named` entry `operation_N` → gsub `opidNhere` (declared params run first, so explicit `opidNhere → operation_N` mappings in eTFL-style metadata are unaffected). Note the shipped `exampleARS_5_documentref.json` does NOT repro the bug — its manifest declares the opid params explicitly; the contract test in `test-method-reference.R` builds an ARS pointing at the bundled `method-library.json` instead.
