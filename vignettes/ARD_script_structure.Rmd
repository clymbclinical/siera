...
One convention to be aware of: `cards` reports proportions on the 0-1 scale (under `stat_name == "p"`), while result patterns express percentages - so those rows are multiplied by 100 before formatting (a raw `0.756` with pattern `( XX.X)` becomes `( 75.6)`). p-values (`stat_name == "p.value"`) are left as-is.
